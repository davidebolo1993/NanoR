...
#' @title Generates a metadata table for your MinION .fast5 files
#' @description NanoTableM generates a table that contains useful informations for each read of the "pass" .fast5 files folder given to NanoPrepareM function. As NanoTableM can take some time (it depends on the number of reads it has to deal with), this function can be accelerated using multiple cores
#' @param NanoPrepareMList Object of class list returned by NanoPrepareM
#' @param DataOut Where the table will be saved. Do not use a directory that already contains a NanoTableM (or NanoTableG) result
#' @param Cores Number of cores to be used: 1 by default
#' @param GCC Logical. If TRUE, NanoTableM computes GC content for each read. Default to TRUE
#' @return Table with 7 columns
#' @examples
#' #do not run
