...
#' @description NanoStatsG plots statistics for passed .fast5 files and returns 4 tables used by NanoCompare
#' @param NanoPrepareGList Object of class list returned by NanoPrepareG
#' @param NanoGTable Table returned by NanoTableG
#' @param DataOut Where NanoStatsG results will be saved. Use the same directory specified for NanoTableG function and be sure that it doesn't already contain NanoStatsM (or NanoStatsG) results
#' @return Plots: \cr 
#' - Cumulative_Reads_&_Cumulative_Basepairs.pdf; \cr 
#' - Reads_Basepairs_Length_Quality.pdf; \cr 
#' - Length_versus_Quality.pdf; \cr 
#' - Pass_Fail_Skip_and_GC_Content.pdf or Pass_Fail_Skip_NO_GC_Content.pdf; \cr 
#' - Channels_Activity.pdf or Channels_and_Muxes_Activity.pdf. Not-working channels and muxes are grey-colored.
#' @examples
#' #do not run
  ...
  if (is.na(NanoPrepareGList[[1]][1]) == FALSE & is.na(NanoPrepareGList[[3]][1])) {
    ...
    for (iii in 1:length(Channels_Number)) {
      ...
      if (length(Ind_Chann) == 0) {
        ...
        #Table_HDF5_Reordered<-rbind(Table_HDF5_Reordered,Table_HDF5_Re) slow for very large set
      }
    ...
    Cumulative_Reads_Plot<-ggplot(data0.1, aes(x=x, y=y, group=group, fill=group)) +
      geom_line(size=.5) + 
      scale_x_continuous(name="Time (hours)", breaks=(seq(0,Run_Duration,2)))+
    ...
    Length_Per_Hour_Plot<-ggplot(data3, aes(x=x, y=y, group=group, fill=group)) +
      geom_line(size=.5) + 
      scale_x_continuous(name="Time (hours)", breaks=(seq(0,Run_Duration,2)))+
      scale_y_continuous(name="Length (bp)", breaks=c(2,3,4,5,6), labels=c("100","1000","10000","100000","1000000"))+
    ...
    if (length(Template_Length) <= limit) {
      ...
      message("Too many points to plot, rescaling ...")
      tmp <- cbind(Template_Length,Quality_Score)
      Tot <- data.frame(tmp[sample(nrow(tmp), limit), ])
      colnames(Tot) <- c("Template_Length","Quality_Score")      


    }

    ScatterTheme <- list(labs(x="Length (bp)",y="Quality (Phred)"),theme_bw(), theme(legend.position=c(1,0),legend.justification=c(1,0), legend.background=element_blank(),legend.direction="horizontal", legend.title=element_text(face="bold.italic"),axis.title=element_text(face="italic")))
