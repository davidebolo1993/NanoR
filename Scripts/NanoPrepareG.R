...
#' @param BasecalledFast5 Logical. TRUE if dealing with basecalled .fast5 files. Defaulto to FALSE
#' @param Data Path to GridION X5 folder containing .fastq and sequencing summary files (if BasecalledFast5 = FALSE) or to basecalled .fast5 files (if BasecalledFast5 = TRUE)
#' @param DataSkip Path to skipped .fast5 files folder
#' @param DataFail Path to failed .fast5 files folder
#' @param Cores Number of cores to be used to accelerate sequencing summary files reading (useful only if BasecalledFast5 = FALSE)
#' @param Label Label used, together with the Flow Cell identifier extracted from the inputted data, to identify your experiment: do not use underscore characters ("_").
#' @details NanoPrepareg can find desired inputs recursively. DataSkip and DataFail can be omitted.
