...
#' @param Label Label to use, together with the Flow Cell identifier, to identify the experiment
...
FastqFilterG<-function(Data,DataOut,FASTQTOT=FALSE,FASTA=FALSE,Cores=1,Label, Minquality=7) {
  ...
  TablePass<-which(as.numeric(SummaryTable[,3]) >= Minquality)
  PassTable<-SummaryTable[TablePass,]
